...
try:
    ...
    raise ImportError(
        "\033[1;5;31m argcomplete module not found, run: python3 install_requirements.py \033[0m"
    )
    ...
    def checkFn(value):
        ...
        if minVal <= ivalue <= maxVal:
            ...
            raise argparse.ArgumentTypeError(
                "{} is an invalid int value, must be in range {}..{}".format(
                    value, minVal, maxVal
                )
            )
    ...
    def _fun(option):
        ...
        if len(optionList) not in [1, 2]:
            ...
            try:
                ...
                raise argparse.ArgumentTypeError(
                    "In option: {0} {1} is not in a correct format!".format(
                        option, optionList[1]
                    )
                )
...
orientationChoices = list(
    filter(lambda var: var[0].isupper(), vars(dai.CameraImageOrientation))
)
...
def orientationCast(arg):
    if not hasattr(dai.CameraImageOrientation, arg):
        raise argparse.ArgumentTypeError(
            "Invalid camera orientation specified: '{}'. Available: {}".format(
                arg, orientationChoices
            )
        )
...
openvinoVersions = list(
    map(
        lambda name: name.replace("VERSION_", ""),
        filter(lambda name: name.startswith("VERSION_"), vars(dai.OpenVINO.Version)),
    )
)
_streamChoices = (
    "nnInput",
...
colorMaps = list(
    map(
        lambda name: name[len("COLORMAP_") :],
        filter(lambda name: name.startswith("COLORMAP_"), vars(cv2)),
    )
)
...
def parseArgs():
    ...
    parser.add_argument(
        "-cam",
        "--camera",
        choices=[Previews.left.name, Previews.right.name, Previews.color.name],
        default=Previews.color.name,
        help="Use one of DepthAI cameras for inference (conflicts with -vid)",
    )
    parser.add_argument(
        "-vid",
        "--video",
        type=str,
        help="Path to video file (or YouTube link) to be used for inference (conflicts with -cam)",
    )
    parser.add_argument(
        "-dd", "--disableDepth", action="store_true", help="Disable depth information"
    )
    parser.add_argument(
        "-dnn",
        "--disableNeuralNetwork",
        action="store_true",
        help="Disable neural network inference",
    )
    parser.add_argument(
        "-cnnp", "--cnnPath", type=Path, help="Path to cnn model directory to be run"
    )
    parser.add_argument(
        "-cnn",
        "--cnnModel",
    ...
    parser.add_argument(
        ...
        "--shaves",
        type=int,
        help="Number of MyriadX SHAVEs to use for neural network blob compilation",
    )
    parser.add_argument(
        "-cnnsize",
        "--cnnInputSize",
    ...
    parser.add_argument(
        "-dct",
        "--disparityConfidenceThreshold",
        default=245,
        type=checkRange(0, 255),
    ...
    parser.add_argument(
        "-scale",
        "--scale",
        type=_comaSeparated(default=0.5, cast=float),
        nargs="+",
    ...
    parser.add_argument(
        ...
        default=[],
        nargs="+",
        choices=_streamChoices,
        help="Choose which previews to show. Default: %(default)s",
    )
    parser.add_argument(
        "--report",
        nargs="+",
        default=[],
        choices=["temp", "cpu", "memory"],
        help="Display device utilization data",
    )
    parser.add_argument(
        "--reportFile", help="Save report data to specified target file in CSV format"
    )
    parser.add_argument(
        "-sync",
        "--sync",
    ...
    parser.add_argument(
        ...
        "--callback",
        type=Path,
        default=projectRoot / "callbacks.py",
        help="Path to callbacks file to be used. Default: %(default)s",
    )
    parser.add_argument(
        "--openvinoVersion",
        type=str,
        choices=openvinoVersions,
    ...
    parser.add_argument(
        "-enc",
        "--encode",
        type=_comaSeparated(default=30.0, cast=float),
        nargs="+",
        default=[],
    ...
    parser.add_argument(
        ...
        type=Path,
        default=projectRoot,
        help="Path to directory where to store encoded files. Default: %(default)s",
    )
    parser.add_argument(
        "-xls", "--xlinkChunkSize", type=int, help="Specify XLink chunk size"
    )
    parser.add_argument(
        "-camo",
        "--cameraOrientation",
        type=_comaSeparated(default="AUTO", cast=orientationCast),
        nargs="+",
        default=[],
        help=(
            "Define cameras orientation (available: {}) \n"
            "Format: camera_name,camera_orientation \n"
            "Example: -camo color,ROTATE_180_DEG right,ROTATE_180_DEG left,ROTATE_180_DEG"
        ).format(", ".join(orientationChoices)),
    )
    parser.add_argument(
        "--cameraControlls",
        action="store_true",
    ...
    parser.add_argument(
        "--cameraSensitivity", type=int, help="Specify camera sensitivity"
    )
    parser.add_argument(
        "--cameraSaturation", type=checkRange(-10, 10), help="Specify image saturation"
    )
    parser.add_argument(
        "--cameraContrast", type=checkRange(-10, 10), help="Specify image contrast"
    )
    parser.add_argument(
        "--cameraBrightness", type=checkRange(-10, 10), help="Specify image brightness"
    )
    parser.add_argument(
        "--cameraSharpness", type=checkRange(0, 4), help="Specify image sharpness"
    )
