...
def setPolygonCoordinates(height, width):
    ...
    p_coordinates = [
        ...
        [
            [width - horizontal_shift, 0],
            [width - horizontal_shift, height],
            [width // 2 - horizontal_shift, height - slope],
            [width // 2 - horizontal_shift, slope],
        ],
        [
            [width - horizontal_shift * 2 + margin, 0],
            [width - horizontal_shift * 2 + margin, height],
            [width // 2 - horizontal_shift * 2 + margin, height - slope],
            [width // 2 - horizontal_shift * 2 + margin, slope],
        ],
        [
        ...
        [
            [0, height - vertical_shift],
            [width, height - vertical_shift],
            [width - slope, height // 2 - vertical_shift],
            [slope, height // 2 - vertical_shift],
        ],
        [
            [0, height - vertical_shift * 2 + margin],
            [width, height - vertical_shift * 2 + margin],
            [width - slope, height // 2 - vertical_shift * 2 + margin],
            [slope, height // 2 - vertical_shift * 2 + margin],
        ],
    ]
...
def image_filename(stream_name, polygon_index, total_num_of_captured_images):
    return "{stream_name}_p{polygon_index}_{total_num_of_captured_images}.png".format(
...
class StereoCalibration(object):
    ...
    def calibrate(
        ...
        print(
            "\tTook %i seconds to run image processing."
        ...
        if self.calibrate_rgb:
            return (
                self.test_epipolar_charuco_lr(filepath),
                self.test_epipolar_charuco_rgbr(filepath),
                self.calib_data,
            )
        else:
        ...
        for im in images:
            ...
            if scale_req and not (
                ...
                if int(expected_height) == req_resolution[0]:
                    ...
                    gray = cv2.resize(
                        gray, req_resolution[::-1], interpolation=cv2.INTER_CUBIC
                    )
                    # print('~~~~~~~~~~ Only resizing.... ~~~~~~~~~~~~~~~~')
                else:
                    ...
                    dest_res = (
                        int(gray.shape[1] * scale_width),
                        int(gray.shape[0] * scale_width),
                    )
                    # print("destination resolution------>")
                    # print(dest_res)
                    gray = cv2.resize(gray, dest_res, interpolation=cv2.INTER_CUBIC)
                    # print("scaled gray shape")
                    # print(gray.shape)
                    if gray.shape[0] < req_resolution[0]:
                        raise RuntimeError(
                            "resizeed height of rgb is smaller than required. {0} < {1}".format(
                                gray.shape[0], req_resolution[0]
                            )
                        )
            ...
            marker_corners, ids, refusd, recoverd = cv2.aruco.refineDetectedMarkers(
                gray, self.board, marker_corners, ids, rejectedCorners=rejectedImgPoints
            )
            print(
                "{0} number of Markers corners detected in the above image".format(
                    len(marker_corners)
                )
            )
    ...
    def calibrate_charuco3D(self, filepath):
        ...
        if self.cameraModel == "perspective":
            ...
            ret_l, self.M1, self.d1, rvecs, tvecs = self.calibrate_fisheye(
                allCorners_l, allIds_l, self.img_shape
            )
            ret_r, self.M2, self.d2, rvecs, tvecs = self.calibrate_fisheye(
                allCorners_r, allIds_r, self.img_shape
            )
        ...
        if self.cameraModel == "perspective":
            ...
            (
                ...
                self.M1, self.d1, self.M2, self.d2, self.img_shape, self.R, self.T
            )
        else:
    ...
    def calibrate_camera_charuco(self, allCorners, allIds, imsize):
        ...
        if imsize[1] < 1100:
            cameraMatrixInit = np.array(
                [[857.1668, 0.0, 643.9126], [0.0, 856.0823, 387.56018], [0.0, 0.0, 1.0]]
            )
        else:
            cameraMatrixInit = np.array(
                [
                    [3819.8801, 0.0, 1912.8375],
    ...
    def calibrate_fisheye(self, allCorners, allIds, imsize):
        ...
        cameraMatrixInit = np.array(
            [[500, 0.0, 643.9126], [0.0, 500, 387.56018], [0.0, 0.0, 1.0]]
        )
        ...
        term_criteria = (cv2.TERM_CRITERIA_COUNT + cv2.TERM_CRITERIA_EPS, 100, 1e-5)
    ...
    def rgb_calibrate(self, filepath):
        ...
        if not images_rgb_pth.exists():
            raise RuntimeError(
                "RGB dataset folder not found!! To skip rgb calibration use -drgb argument"
            )
        ...
        (
            ...
            imsize_rgb_scaled,
            _,
        ) = self.analyze_charuco(images_rgb, scale_req=True, req_resolution=(720, 1280))
        ...
        ret, _, _, _, _, self.R_rgb, self.T_rgb, E, F = cv2.stereoCalibrate(
            self.objpoints_rgb_r,
            self.imgpoints_rgb,
            self.imgpoints_rgb_right,
            self.M3_scaled,
            self.d3_scaled,
            self.M2_rgb,
            self.d2_rgb,
            self.img_shape_rgb_scaled,
            criteria=stereocalib_criteria,
            flags=flags,
        )
    ...
    def test_epipolar_charuco_lr(self, dataset_dir):
        ...
        for i, image_data_pair in enumerate(image_data_pairs):
            ...
            marker_corners_l, ids_l, _, _ = cv2.aruco.refineDetectedMarkers(
                image_data_pair[0],
                self.board,
                marker_corners_l,
                ids_l,
            ...
            marker_corners_r, ids_r, _, _ = cv2.aruco.refineDetectedMarkers(
                image_data_pair[1],
                self.board,
                marker_corners_r,
                ids_r,
            ...
            print(
                "Average Epipolar Error per image on host in "
                + img_pth.name
                + " : "
    ...
    def test_epipolar_charuco_rgbr(self, dataset_dir):
        ...
        for image_rgb, image_right in zip(images_rgb, images_right):
            ...
            dest_res = (
                int(img_rgb.shape[1] * scale_width),
