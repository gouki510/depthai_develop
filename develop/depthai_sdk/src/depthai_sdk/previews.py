...
class PreviewDecoder:
    ...
    def nnInput(packet, manager=None):
        ...
        if hasattr(manager, "nnSource") and manager.nnSource in (
            Previews.rectifiedLeft.name,
            Previews.rectifiedRight.name,
        ):
    ...
    def color(packet, manager=None):
        ...
        """
        if (
            manager is not None and manager.lowBandwidth and not manager.sync
        ):  # TODO remove sync check once passthrough is supported for MJPEG encoding
    ...
    def depth(depthRaw, manager=None):
        ...
        if getattr(manager, "_depthConfig", None) is None:
            raise RuntimeError(
                "Depth config has to be provided before decoding depth data"
            )

        maxDisp = manager._depthConfig.getMaxDisparity()
        subpixelLevels = pow(
            2, manager._depthConfig.get().algorithmControl.subpixelFractionalBits
        )
        ...
        if dispScaleFactor is None:
            ...
            focal = getattr(
                manager,
                "focal",
                depthRaw.shape[1] / (2.0 * math.tan(math.radians(fov / 2))),
            )
        ...
        dispFrame = (dispFrame * 255.0 / dispIntegerLevels).astype(np.uint8)
    ...
    def disparity(packet, manager=None):
        ...
        return (
            rawFrame * (manager.dispMultiplier if manager is not None else 255 / 96)
        ).astype(np.uint8)
    ...
    def disparityColor(disparity, manager=None):
        ...
        return cv2.applyColorMap(
            disparity, manager.colorMap if manager is not None else cv2.COLORMAP_JET
        )
