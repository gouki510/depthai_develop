...
class PreviewManager:
    ...
    def collectCalibData(self, device):
        ...
        if leftCam != dai.CameraBoardSocket.AUTO:
            ...
            self.focal = (camInfo.width / 2) / (
                2.0 * math.tan(math.radians(self.fov / 2))
            )
        else:
    ...
    def createQueues(self, device, callback=None):
        ...
        for name in self._display:
            ...
            if name not in (
                ...
                self.outputQueues.append(
                    device.getOutputQueue(name=name, maxSize=1, blocking=False)
                )

        if (
            Previews.disparityColor.name in self._display
            and Previews.disparity.name not in self._display
        ):
            self.outputQueues.append(
                device.getOutputQueue(
                    name=Previews.disparity.name, maxSize=1, blocking=False
                )
            )
        if (
            Previews.depth.name in self._display
            and Previews.depthRaw.name not in self._display
        ):
            self.outputQueues.append(
                device.getOutputQueue(
                    name=Previews.depthRaw.name, maxSize=1, blocking=False
                )
            )
    ...
    def prepareFrames(self, blocking=False, callback=None):
        ...
        for queue in self.outputQueues:
            ...
            if packet is not None:
                ...
                if frame is None:
                    print(
                        "[WARNING] Conversion of the {} frame has failed! (None value detected)".format(
                            queue.getName()
                        )
                    )
                    continue
                if self.scale is not None and queue.getName() in self.scale:
                    ...
                    frame = cv2.resize(
                        frame,
                        (
                            int(w * self.scale[queue.getName()]),
                            int(h * self.scale[queue.getName()]),
                        ),
                ...
                if self._mouseTracker is not None:
                    if queue.getName() == Previews.disparity.name:
                        rawFrame = (
                            packet.getFrame()
                            if not self.lowBandwidth
                            else cv2.imdecode(packet.getData(), cv2.IMREAD_GRAYSCALE)
                        )
                        self._mouseTracker.extractValue(
                            Previews.disparity.name, rawFrame
                        )
                        self._mouseTracker.extractValue(
                            Previews.disparityColor.name, rawFrame
                        )
                    if queue.getName() == Previews.depthRaw.name:
                        rawFrame = (
                            packet.getFrame()
                        )  # if not self.lowBandwidth else cv2.imdecode(packet.getData(), cv2.IMREAD_UNCHANGED) TODO uncomment once depth encoding is possible
                        self._mouseTracker.extractValue(
                            Previews.depthRaw.name, rawFrame
                        )
                ...
                if (
                    queue.getName() == Previews.disparity.name
                    and Previews.disparityColor.name in self._display
                ):
                    if self._fpsHandler is not None:
                        self._fpsHandler.tick(Previews.disparityColor.name)
                    self._rawFrames[
                        Previews.disparityColor.name
                    ] = Previews.disparityColor.value(frame, self)

                if (
                    queue.getName() == Previews.depthRaw.name
                    and Previews.depth.name in self._display
                ):
                    if self._fpsHandler is not None:
                        self._fpsHandler.tick(Previews.depth.name)
                    self._rawFrames[Previews.depth.name] = Previews.depth.value(
                        frame, self
                    )
            ...
            for name in self._rawFrames:
                ...
                if name == Previews.depthRaw.name:
                    newFrame = cv2.normalize(
                        newFrame, None, 255, 0, cv2.NORM_INF, cv2.CV_8UC1
                    )
