...
class PipelineManager:
    ...
    def _mjpegLink(self, node, xout, nodeOutput):
        print(
            "Creating MJPEG link for {} node and {} xlink stream...".format(
                node.getName(), xout.getStreamName()
            )
        )
        ...
        if isinstance(node, dai.node.ColorCamera):
            if node.video == nodeOutput:
                ...
                videnc.setDefaultProfilePreset(
                    node.getFps(), dai.VideoEncoderProperties.Profile.MJPEG
                )
            ...
            cameraNode = getattr(
                self.nodes, "monoLeft", getattr(self.nodes, "monoRight", None)
            )
            if cameraNode is None:
                raise RuntimeError(
                    "Unable to find mono camera node to determine frame size!"
                )
            videnc.setDefaultProfilePreset(
                cameraNode.getFps(), dai.VideoEncoderProperties.Profile.MJPEG
            )
            nodeOutput.link(videnc.input)
            ...
            raise NotImplementedError(
                "Unable to create mjpeg link for encountered node type: {}".format(
                    type(node)
                )
            )
    ...
    def createColorCam(
        ...
        if xout:
            if self.lowBandwidth:
                self._mjpegLink(
                    self.nodes.camRgb, self.nodes.xoutRgb, self.nodes.camRgb.video
                )
            else:
    ...
    def createLeftCam(
        ...
        if xout:
            if self.lowBandwidth:
                self._mjpegLink(
                    self.nodes.monoLeft, self.nodes.xoutLeft, self.nodes.monoLeft.out
                )
            else:
    ...
    def createRightCam(
        ...
        if xout:
            if self.lowBandwidth:
                self._mjpegLink(
                    self.nodes.monoRight, self.nodes.xoutRight, self.nodes.monoRight.out
                )
            else:
    ...
    def createDepth(
        ...
        if not hasattr(self.nodes, "monoLeft"):
            raise RuntimeError(
                "Left mono camera not initialized. Call createLeftCam(res, fps) first!"
            )
        if not hasattr(self.nodes, "monoRight"):
            raise RuntimeError(
                "Right mono camera not initialized. Call createRightCam(res, fps) first!"
            )
        ...
        if useDepth:
            ...
            if False:
                self._mjpegLink(
                    self.nodes.stereo, self.nodes.xoutDepth, self.nodes.stereo.depth
                )
            else:
        ...
        if useDisparity:
            ...
            if self.lowBandwidth:
                self._mjpegLink(
                    self.nodes.stereo,
                    self.nodes.xoutDisparity,
                    self.nodes.stereo.disparity,
                )
            else:
        ...
        if useRectifiedLeft:
            ...
            if False:
                self._mjpegLink(
                    self.nodes.stereo,
                    self.nodes.xoutRectLeft,
                    self.nodes.stereo.rectifiedLeft,
                )
            else:
        ...
        if useRectifiedRight:
            ...
            if False:
                self._mjpegLink(
                    self.nodes.stereo,
                    self.nodes.xoutRectRight,
                    self.nodes.stereo.rectifiedRight,
                )
            else:
                self.nodes.stereo.rectifiedRight.link(self.nodes.xoutRectRight.input)

    def _updateCamConfig(
        self,
        configRef: dai.CameraControl,
        cameraName,
        device,
        exposure=None,
        sensitivity=None,
        ...
        if any([exposure, sensitivity]):
            if not all([exposure, sensitivity]):
                raise RuntimeError(
                    'Both "exposure" and "sensitivity" arguments must be provided'
                )
    ...
    def updateDepthConfig(
        self, device, dct=None, sigma=None, median=None, lrc=None, lrcThreshold=None
    ):
        """
    ...
    def createEncoder(self, cameraName, encFps=30, encQuality=100):
        ...
        if cameraName not in allowedSources:
            raise ValueError(
                "Camera param invalid, received {}, available choices: {}".format(
                    cameraName, allowedSources
                )
            )
        ...
        if cameraName == Previews.color.name:
            if not hasattr(self.nodes, "camRgb"):
                raise RuntimeError(
                    "RGB camera not initialized. Call createColorCam(res, fps) first!"
                )
            encProfile = dai.VideoEncoderProperties.Profile.H264_MAIN
