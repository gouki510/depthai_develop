...
class NNetManager:
    ...
    def readConfig(self, path):
        ...
        with configPath.open() as f:
            ...
            if "openvino_version" in self._config:
                self.openvinoVersion = getattr(
                    dai.OpenVINO.Version,
                    "VERSION_" + self._config.get("openvino_version"),
                )
            ...
            self._confidence = self._metadata.get(
            ...
            if "handler" in self._config:
                ...
                if not callable(getattr(self._handler, "draw", None)) or not callable(
                    getattr(self._handler, "decode", None)
                ):
                    raise RuntimeError(
                        "Custom model handler does not contain 'draw' or 'decode' methods!"
                    )
    ...
    def _normFrame(self, frame):
        if not self._fullFov:
            ...
            return np.zeros(
                (int(self.inputSize[1] * scaleF), int(self.inputSize[0] * scaleF))
            )
        else:
            return frame
    ...
    def createNN(
        ...
        if source not in self.sourceChoices:
            raise RuntimeError(
                f"Source {source} is invalid, available {self.sourceChoices}"
            )
        if self.inputSize is None:
            raise RuntimeError(
                "Unable to determine the nn input size. Please use --cnnInputSize flag to specify it in WxH format: -nnSize <width>x<height>"
            )
        ...
        if self._nnFamily == "mobilenet":
            nodes.nn = (
                pipeline.createMobileNetSpatialDetectionNetwork()
                if useDepth
                else pipeline.createMobileNetDetectionNetwork()
            )
            ...
            nodes.nn = (
                ...
                if useDepth
                else pipeline.createYoloDetectionNetwork()
            )
            nodes.nn.setConfidenceThreshold(self._confidence)
    ...
    def getLabelText(self, label):
        ...
        if self._labels is None:
            ...
            print(
                f"Label of ouf bounds (label index: {label}, available labels: {len(self._labels)}"
            )
            return str(label)
            ...
            if self._handler is not None:
                ...
                try:
                    ...
                    print(
                        "Received NN packet: ",
                        ", ".join(
                            [f"{key}: {value.shape}" for key, value in data.items()]
                        ),
                    )
    ...
    def _drawCount(self, source, decodedData):
        ...
        cntList = list(
            filter(
                lambda x: self.getLabelText(x.label) == self._countLabel, decodedData
            )
        )
        ...
        if self._outputFormat == "detection":
            ...
            def drawDetection(frame, detection):
                ...
                cv2.rectangle(
                    ...
                    (bbox[0], bbox[1]),
                    (bbox[2], bbox[3]),
                    self._bboxColors[detection.label],
                    2,
                )
                cv2.rectangle(
                    frame,
                ...
                cv2.putText(
                    frame,
                    self.getLabelText(detection.label),
                    (bbox[0] + 5, bbox[1] - 10),
                    self._textType,
                    0.5,
                    (0, 0, 0),
    ...
    def sendInputFrame(self, frame, seqNum=None):
        ...
        if self.inputQueue is None:
            raise RuntimeError(
                "Unable to send image, no input queue is present! Call `createQueues(device)` first!"
            )
