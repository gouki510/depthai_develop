...
class EncodingManager:
    ...
    def createDefaultQueues(self, device):
        ...
        for cameraName, node in self._encodingNodes.items():
            self._encodingQueues[cameraName] = device.getOutputQueue(
                cameraName + "EncXout", maxSize=30, blocking=True
            )
            suffix = (
                ".h265"
                if node.getProfile() == dai.VideoEncoderProperties.Profile.H265_MAIN
                else ".h264"
            )
            self._encodingFiles[cameraName] = (
                (self.encodeOutput / cameraName).with_suffix(suffix).open("wb")
            )
    ...
    def close(self):
        ...
        def printManual():
            ...
            for name, file in self._encodingFiles.items():
                print(
                    cmd.format(
                        self._encodingNodes[name].getFrameRate(),
                        file.name,
        ...
        try:
            ...
            for name, file in self._encodingFiles.items():
                print(
                    "Produced file: {}".format(str(Path(file.name).with_suffix(".mp4")))
                )
        except ImportError:
